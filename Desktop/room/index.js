...
document.getElementById('submit').onclick = (event) => {
  ...
  const gender = document.querySelector('input[name="input"]:checked').value
  ...
  if (user.fname == '') {
    window.alert(`enter your role`)
    return
  }

  if (rooms[room].length >= 5) {
    window.alert(`Room ${room} is full, maximum facilitators reached`)
    return
  }
